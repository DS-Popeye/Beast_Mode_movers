  ...
  return (
    <header
      className={`fixed top-0 left-0 w-full z-50 bg-white transition-all duration-300 ${
        scrolled ? 'shadow-md border-b border-neutral-100' : 'border-b border-neutral-200'
      }`}
    >
      <div className="px-6 lg:px-16 flex items-center justify-between h-[76px]">
          ...
          {navLinks.map((link) => {
            ...
            return (
              <Link
                key={link.label}
                to={link.href}
                className={`text-[13px] font-semibold tracking-wide transition-colors whitespace-nowrap cursor-pointer ${
                  isActive ? 'text-[#58C016]' : 'text-neutral-700 hover:text-[#58C016]'
                }`}
              >
                {link.label}
              </Link>
        ...
        <div className="hidden lg:flex items-center gap-3">
          ...
          <Link
            to="/#contact"
            className="bg-[#70DC28] text-[#0f0f0f] px-5 py-2.5 rounded-md font-bold text-[13px] whitespace-nowrap hover:bg-[#58C016] transition-colors cursor-pointer"
          >
            Get Free Quote →
          </Link>
        </div>
